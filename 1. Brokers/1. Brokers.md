...
Par exemple, si nous avons un broker de stockage qui fournit toutes les opérations CRUD pour les modèles `Student` et `Teacher`, l'organisation des fichiers doit être la suivante :
...
Les brokers d'entités ne peuvent être appelés que par des services voisins de broker, simplement parce qu'ils nécessitent un niveau de validation qui doit être effectué sur les données qu'ils reçoivent ou fournissent avant de continuer.
...
Un bon exemple de brokers de support est le `DateTimeBroker` - un broker spécialement conçu pour éliminer la forte dépendance de la couche métier à l'heure de la date du système.
...
Voici une implémentation réelle d'un broker de stockage complet pour toutes les opérations CRUD pour l'entité `Student` :
...
Faire cela au niveau du broker nécessitera de pousser la logique métier dans ce domaine, ce qui est complètement en dehors de l'objectif de ce composant.
...
Les brokers ne sont que la *première* couche d'abstraction, mais pas la seule - les services voisins du broker sont responsables de la conversion des exceptions natives provenant d'un broker en un modèle d'exception plus local qui peut être géré et traité en interne dans la logique métier royaume.
...
Étant donné que les brokers doivent posséder leurs propres configurations, il était plus logique de procéder à une partialisation lorsque cela était possible pour éviter de reconfigurer chaque broker de stockage pour chaque entité.
