...
Un bon exemple de validation de dépendance est lorsque nous appelons un courtier pour récupérer une entité particulière par son identifiant. Si l'entité renvoyée est introuvable ou si le courtier d'API renvoie une erreur `NotFound`, le service de base doit alors envelopper cette erreur dans une `ValidationException` et arrête tous les processus suivants.
...
Le test ci-dessus nous oblige à lever une exception localisée comme dans `NotFoundStudentException` lorsque le courtier de stockage ne renvoie aucune valeur pour le `studentId` donné, puis à l'envelopper ou à le catégoriser dans `StudentValidationException`.
...
Écrivons un test d'échec pour vérifier si nous lançons une `DependencyValidationException` si le modèle `Student` existe déjà dans le stockage avec le courtier de stockage lançant une `DuplicateKeyException` comme résultat natif de l'opération.
